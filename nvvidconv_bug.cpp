//usr/bin/g++ $0 -o nvvidconv_bug -lEGL -lGL `pkg-config --cflags gstreamer-1.0` `pkg-config --libs gstreamer-1.0` && ./nvvidconv_bug; exit
// This is a self-running .cpp file.  chmod +x this file.  Then you can run it like a shell script.
// Based on https://github.com/DaneLLL/gstreamer_eglstreamsrc and https://github.com/DaneLLL/simpleEGLStreams_producer
...
int main(int argc, char** argv) {
  ...
  for (int loopCnt=0; loopCnt < 10; loopCnt++)
  {
...

    {
        ...
        if ((eglDisplay = eglGetDisplay(EGL_DEFAULT_DISPLAY)) == EGL_NO_DISPLAY) return 1;
        if (!eglInitialize(eglDisplay, 0, 0)) return 1;
        EGLint streamAttrMailboxMode[] = { EGL_NONE };
        if ((eglStream = EglCreateStreamKHR(eglDisplay, streamAttrMailboxMode)) == EGL_NO_STREAM_KHR) return 1;
        if (!EglStreamAttribKHR(eglDisplay, eglStream, EGL_CONSUMER_LATENCY_USEC_KHR, 16000)) return 1;
        if (!EglStreamAttribKHR(eglDisplay, eglStream, EGL_CONSUMER_ACQUIRE_TIMEOUT_USEC_KHR, 16000)) return 1;
    ...
    }
    {
        ...
        if(!GST_IS_ELEMENT(videoSource)) return 1;
    ...
    }
    {
        ...
        EGLint n = 0;
        if (!eglChooseConfig(eglDisplay, cfg_attr, &eglConfig, 1, &n) || !n) return 1;
        EGLint ctx_attr[] = { EGL_CONTEXT_CLIENT_VERSION, 2, EGL_NONE };
        eglBindAPI(EGL_OPENGL_ES_API);
        if ((eglContext = eglCreateContext(eglDisplay, eglConfig, EGL_NO_CONTEXT, ctx_attr)) == EGL_NO_CONTEXT) return 1;
        EGLint srf_attr[] = { EGL_WIDTH, width, EGL_HEIGHT, height, EGL_NONE };
        if ((eglSurface = EglCreateStreamProducerSurfaceKHR(eglDisplay, eglConfig, eglStream, srf_attr)) == EGL_NO_SURFACE) return 1;
...
//        g_main_loop_unref(main_loop);
    ...
    }
    {
        ...
        if(!EglQueryStreamKHR(eglDisplay, eglStream, EGL_STREAM_STATE_KHR, &streamState)) return 1;
